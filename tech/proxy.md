...
### 1. 相关概念
(1). **代理**：在某些情况下，我们不希望或是不能直接访问对象 A，而是通过访问一个中介对象 B，由 B 去访问 A 达成目的，这种方式我们就称为代理。  
...
(2). **静态代理**：代理类在程序运行前已经存在的代理方式称为静态代理。  
...
```
上面`ClassA`是委托类，`ClassB`是代理类，`ClassB`中的函数都是直接调用`ClassA`对象，并且隐藏了`Class`的`operateMethod3()`函数。  

静态代理中代理类和委托类也常常继承同一父类或实现同一接口。  

(3). **动态代理**：代理类在程序运行前不存在、运行时由程序动态生成的代理方式称为动态代理。  
Java 提供了动态代理的实现方式，可以在运行时刻动态生成代理类。这种代理方式的一大好处是很方便对代理类的函数做统一或特殊处理，如记录所有函数执行时间、所有函数执行前添加验证判断、对某个特殊函数进行特殊操作，而不用像静态代理方式一样修改每个函数。  

`静态代理`比较简单，本文不做介绍，重点介绍`动态代理`。  
...
`InvocationHandler`实际是负责连接代理类和委托类的中间类必须实现的接口。其中只有一个  
```java
public Object invoke(Object proxy, Method method, Object[] args)
```
函数需要去实现，参数：  
`proxy`表示`2.3 通过 Proxy.newProxyInstance() 生成的代理类对象`。  
`method`表示代理对象被调用的函数。  
...
```
`loader`表示类加载器  
`interfaces`表示委托类的接口，生成委托类时需要实现这些接口  
...
```  
从中我们可以看出动态生成的代理类是以`com.sun.proxy`为包名，`$Proxy`为类名前缀，继承自`Proxy`，并且实现了`Proxy.newProxyInstance(…)`第二个参数传入的所有接口的类。  
    ...
    throws IllegalArgumentException
{
    ...
    /*
     * loaderToCache 是个双层的 Map，第一层 key 为 ClassLoader，第二层 key 为 上面的 List，value 为代理类的弱引用
     */
    Map cache;
     ...
     *     null 表示不在缓存中且没有开始生成，添加标记到缓存中待生成代理类
     */
    synchronized (cache) {
        do {
    ...
    try {
        ...
        /*
         * 如果 interfaces 中存在非 public 的接口，则所有非 public 接口必须在同一包下面，且后续生成的代理类也需要在该下面
         */
        for (int i = 0; i < interfaces.length; i++) {
        ...
        {
            // 生成代理类的类名，jdk 1.6 版本中缺少对这个生成类已经存在的处理。
            long num;
...
**函数中几个注意的地方：**  
* 代理类的缓存 key 为接口名对应的 List，接口顺序不同表示不同的 key 即不同缓存类。  
* 代理类如果在 ClassLoader 中已经存在的情况没有做处理。  
* 如果 interfaces 中存在非 public 的接口，则所有非 public 接口必须在同一包下面，且后续生成的代理类也需要在该下面。  
* 可以开启 "sun.misc.ProxyGenerator.saveGeneratedFiles" 开关，保存动态类到目的地址。  
...
作用：简化网络请求操作。  
一般情况下每个网络请求我们都需要调用一次`HttpURLConnection`或者`HttpClient`进行请求，或者像 [Volley](https://github.com/android-cn/android-open-project-analysis/tree/master/volley) 一样丢进等待队列中，Retrofit 极大程度简化了这些操作，示例代码如下：  
```java
...
```
即可，`Retrofit`的原理就是基于动态代理，它同时用到了 [注解](https://github.com/android-cn/android-open-project-analysis/blob/master/tech/annotation.md) 的原理，本文不做深入介绍，具体请等待 [Retrofit  实现原理解析](https://github.com/android-cn/android-open-project-analysis/tree/master/retrofit) 完成。  